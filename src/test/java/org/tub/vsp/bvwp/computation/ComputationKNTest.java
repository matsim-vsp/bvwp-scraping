...
//    @Disabled
    ...
    void test() {
        ...
        {
            double nkv = nkvOhneKR_induz(new Modifications(co2PriceBVWP, 0., 1, 1., 1. ), amounts, benefitsAndInvestmentCosts, benefitsAndInvestmentCosts.benefit_all);
            Assertions.assertEquals( nkv_orig, nkv, 0.001 );
        }

        // doppelte Investitionskosten:
        {
            double nkv = nkvOhneKR_induz(new Modifications( co2PriceBVWP, 0., 2., 1., 1. ), amounts, benefitsAndInvestmentCosts, benefitsAndInvestmentCosts.benefit_all);
            Assertions.assertEquals(nkv_orig/2, nkv, 0.001);
        }

        // neuer CO2-Preis:
        {
            double nkv = nkvOhneKR_induz(new Modifications( co2Price796, 0., 1., 1., 1. ), amounts, benefitsAndInvestmentCosts, benefitsAndInvestmentCosts.benefit_all);
            final double delta_nkv = (benefitsAndInvestmentCosts.co2_betrieb + benefitsAndInvestmentCosts.co2_infra) * (co2Price796 / co2PriceBVWP - 1.) / benefitsAndInvestmentCosts.investmentCosts_Barwert;
            Assertions.assertEquals(nkv_orig+delta_nkv, nkv, 0.001);
        }
        // neuer CO2-Preis & eMob:
        {
            double nkv = nkvOhneKR_induz(new Modifications( co2Price796, 0., 1., 1., 0.1 ), amounts, benefitsAndInvestmentCosts, benefitsAndInvestmentCosts.benefit_all);
            final double delta_b = -benefitsAndInvestmentCosts.co2_infra - benefitsAndInvestmentCosts.co2_betrieb + benefitsAndInvestmentCosts.co2_infra*co2Price796/co2PriceBVWP + benefitsAndInvestmentCosts.co2_betrieb*0.1*co2Price796/co2PriceBVWP;
            final double delta_nkv = delta_b / benefitsAndInvestmentCosts.investmentCosts_Barwert;
        ...
        // induced traffic alone:
        {
            ...
            double nkv = nkvOhneKR_induz( modifications, amounts, benefitsAndInvestmentCosts, benefitsAndInvestmentCosts.benefit_all);
            ...
            double discounted_benefits_per_ton_over_30_years = benefitsAndInvestmentCosts.co2_betrieb / amounts.co2_kfz;

            // yyyyyy the above is still confused; co2_betrieb is per year; benefitsAndInvestmentCosts are in mio and per 30 years.  Need to align!
            ...
            Assertions.assertEquals(nkv_orig + delta_b/benefitsAndInvestmentCosts.investmentCosts_Barwert, nkv, 0.001);
        }
...
//        {
//            double nkv = nkv(new Modifications(5. * 145., amounts.getPkwkm_induz() * 4., 1, 1., 1. ), amounts, benefitsAndInvestmentCosts, baukosten );
